...
namespace
{
    ...
    std::string LogicalKeyFromString(std::string raw)
    {
        ...
        if (key == "24" || key == "175" || key == "EXTENSION" || key == "EXTRA_TIME" || key == "ADD_TIME" || key == "VOLUME_UP")
            return "EXTENSION";
        if (key == "25" || key == "174" || key == "TIMER" || key == "TIME" || key == "CLOCK" || key == "VOLUME_DOWN")
            return "TIMER";
        ...
        if (key == "30" || key == "88" || key == "177" || key == "WARM" || key == "WARMUP" || key == "WARM_UP" || key == "MEDIA_PREVIOUS" || key == "MEDIA_PREVIOUS_TRACK" || key == "MEDIA_REWIND")
            return "WARM_UP";
        if (key == "31" || key == "67" || key == "86" || key == "178" || key == "STOP" || key == "MEDIA_STOP")
            return "STOP";
        if (key == "32" || key == "82" || key == "87" || key == "90" || key == "176" || key == "BREAK" || key == "MEDIA_NEXT" || key == "MEDIA_NEXT_TRACK" || key == "MEDIA_FAST_FORWARD")
            return "BREAK";
    ...
    std::string LogicalKeyFromVirtualKey(VirtualKey key)
    {
        ...
        case 69:
        case 107:
        case 175: // VirtualKey::VolumeUp - APLUS remote EXTENSION / Thêm giờ
            return "EXTENSION";
        case 84:
        case 109:
        case 174: // VirtualKey::VolumeDown - APLUS remote TIMER / Bấm giờ
            return "TIMER";
        case 13:
        case 30:
        case 41:
    ...
    void EmitAcceleratorRemoteEvent(CoreDispatcher const &, AcceleratorKeyEventArgs const &args)
    {
        ...
        // TIMER / EXTENSION are exposed by Windows as VolumeDown / VolumeUp.
...
namespace winrt::billiardsgrade::implementation
{
    ...
    {
        // Listener registration is JS-side only. Keep native startup lightweight.
    }

    void RemoteControlModule::SetEnabled(bool enabled) noexcept
    {
        g_enabled = enabled;
        EmitRemoteStatus(enabled ? "enabled" : "disabled", enabled ? "Remote control enabled" : "Remote control disabled");
