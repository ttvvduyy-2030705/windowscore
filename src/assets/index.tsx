import {Images} from 'types/images';

const images: Images = {
  back: require('./images/logo-back.png'),
  close: require('./images/close.png'),
  default: require('./images/default.png'),
  delete: require('./images/delete.png'),
  doubleArrow: require('./images/double-arrow.png'),
  english: require('./images/english.png'),
  eye: require('./images/eye.png'),
  history: require('./images/history.png'),
  logo: require('./images/logo.png'),
  logoFilled: require('./images/logo-filled.png'),
  logoOutlined: require('./images/logo-outlined.png'),
  logoSmall: require('./images/logo-small.png'),
  offline: require('./images/offline.png'),
  plus: require('./images/plus.png'),
  settings: require('./images/setting.png'),
  share: require('./images/share.png'),
  startGame: require('./images/start-game.png'),
  vietnam: require('./images/vietnam.png'),
  google: require('./images/google.png'),
  logoclb: require('./images/logo.png'),
  videoEditor: require('./images/video-editor.png'),

  game: {
    edit: require('./images/game/edit.png'),
    editPencil: require('./images/game/edit-pencil.png'),
    endMatch: require('./images/game/end-match.png'),
    refresh: require('./images/game/refresh.png'),
    clock: require('./images/game/clock.png'),
    camera: require('./images/game/camera.png'),
    change: require('./images/game/change.png'),
    start: require('./images/game/start.png'),
    soundOn: require('./images/game/sound-on.png'),
    soundOff: require('./images/game/sound-off.png'),
    addTime: require('./images/game/add-time.png'),
    turn: require('./images/game/turn.png'),
  },
  webcam: {
    refresh: require('./images/webcam/refresh.png'),
    delay: require('./images/webcam/delay.png'),
    watch: require('./images/webcam/watch.png'),
  },
};

export default images;
