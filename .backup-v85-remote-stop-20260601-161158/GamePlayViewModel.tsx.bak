...
import {
  ...
  isWindowsFfmpegLocalLiveRunning,
  maskStreamKey,
...
let pendingWindowsYouTubeDrainStopId = 0;
const WINDOWS_YOUTUBE_LIVE_END_DRAIN_MS = 22000;
...
type GameplayLiveRouteParams = {
  ...
  prestartedYouTubeLive?: boolean;
  prestartedYouTubeBroadcastId?: string;
  prestartedYouTubeWatchUrl?: string;
};
...
const GamePlayViewModel = () => {
  ...
  const isWindowsYouTubeLiveOnly =
    Platform.OS === 'windows' && shouldUseYouTubeLive && !saveToDeviceWhileStreaming;
  ...
  const lastExtensionPressAtRef = useRef(0);
  ...
  const appliedPrestartedYouTubeLiveRef = useRef(false);
  ...
  const [isStarted, setIsStarted] = useState(
    routeParams.prestartedYouTubeLive
      ? true
  ...
  useEffect(() => {
    if (!routeParams.prestartedYouTubeLive || appliedPrestartedYouTubeLiveRef.current) {
      return;
    }

    appliedPrestartedYouTubeLiveRef.current = true;
    const broadcastId = String(routeParams.prestartedYouTubeBroadcastId || '').trim();
    activeYouTubeBroadcastIdRef.current = broadcastId;

    console.log('[YouTube Live] restored prestarted live from settings', {
      broadcastId,
      watchUrl: routeParams.prestartedYouTubeWatchUrl || '',
    });
    ...
    setYouTubeNativeCameraLock(true);
    setIsStarted(true);
    setActiveGameplaySessionSync({
      matchSessionId: matchSessionIdRef.current,
      webcamFolderName,
      savedAt: Date.now(),
      source: 'settings-prestarted-youtube-live',
    });
  }, [
    routeParams.prestartedYouTubeLive,
    routeParams.prestartedYouTubeBroadcastId,
    routeParams.prestartedYouTubeWatchUrl,
    webcamFolderName,
  ]);
  ...
  const onRemoteStartToggle = useCallback(() => {
    console.log('[Remote][Start] pressed', {isStarted, isPaused});
    ...
    if (isPaused) {
      console.log('[Remote][Start] resume paused match');
  ...
  const onRemoteStopTimer = useCallback(() => {
    console.log('[Remote][Stop] toggle countdown pause', {
      isStarted,
      isPaused,
      isMatchPaused,
    });
  ...
  const onRemoteBreak = useCallback(() => {
    console.log('[Remote][Break] pressed', {
      isStarted,
      isPaused,
      poolBreakEnabled,
    });
    onPoolBreak();
  }, [isStarted, isPaused, poolBreakEnabled, onPoolBreak]);

  useEffect(() => {
    remoteHandlersRef.current = {
      start: onRemoteStartToggle,
      warmUp: warmUpCountdownTime ? onEndWarmUp : onWarmUp,
      stop: onRemoteStopTimer,
      gameBreak: onRemoteBreak,
  ...
  const onPressGiveMoreTime = useCallback(() => {
    const now = Date.now();
    if (now - lastExtensionPressAtRef.current < 700) {
      console.log('[Extension] blocked: duplicate remote press', {
        elapsedMs: now - lastExtensionPressAtRef.current,
      });
      return;
    }
    lastExtensionPressAtRef.current = now;
    ...
    const currentPlayer = latestPlayerSettings?.playingPlayers?.[currentPlayerIndex];
    ...
    if (!isStarted || !latestPlayerSettings || !baseCountdown) {
  ...
  const onStart = useCallback(async () => {
    ...
    pendingWindowsYouTubeDrainStopId += 1;
    ...
    if (Platform.OS === 'windows' && shouldUseYouTubeLive) {
      const runningStatusBeforeCreate = await getWindowsFfmpegLiveStatus().catch(() => null);
      const runningStatusText = String(runningStatusBeforeCreate?.status || '');
      if (
        isWindowsFfmpegLocalLiveRunning() ||
        runningStatusText === 'live' ||
        runningStatusText === 'starting'
      ) {
        console.log('[YouTube Live] duplicate start ignored: FFmpeg is already running', {
          status: runningStatusText || 'live',
          pid: runningStatusBeforeCreate?.pid,
          activeBroadcastId: activeYouTubeBroadcastIdRef.current || '',
        });
        pendingYouTubeNativeStartRef.current = null;
        shouldStartRecordingRef.current = false;
        pendingStartRecordingRef.current = false;
        ...
        setActiveGameplaySessionSync({
          ...
          webcamFolderName,
          savedAt: Date.now(),
          source: 'windows-youtube-live-duplicate-start-ignored',
        });
        return;
      }

      console.log('[LiveWindowsMode]', {
        selectedMode: 'ffmpeg-local-oauth-decoupled-start',
        startsMatchImmediately: true,
        usesNgrok: false,
        usesMetro: false,
      ...
      pendingYouTubeNativeStartRef.current = null;
      setYoutubeLiveOverlay(null);
      // v31: keep the in-app camera preview mounted while the match starts.
      // The real test machine showed the preview only becomes stable after the
      // gameplay/fullscreen layout settles. Releasing MediaCapture immediately
      // made the app view lose camera, while YouTube still stayed “Sắp diễn ra”.
      // So for FFmpeg live we no longer switch the UI into native-live preview mode.
      setYoutubeLivePreparing(false);
      setYoutubeLivePreviewActive(false);
      setYouTubeNativeCameraLock(false);
      setYouTubeSourceLock(null);

      // Do not block the match start while YouTube creates a broadcast or FFmpeg warms up.
      // YouTube/Render can take many seconds; the game must start immediately.
      setIsStarted(true);
      setActiveGameplaySessionSync({
        matchSessionId: matchSessionIdRef.current,
        webcamFolderName,
        savedAt: Date.now(),
        source: 'windows-youtube-live-start-match-immediately',
      });
      ...
      const secondPlayerName =
        ...
        'Player 2';
      const youtubeTitle = `${firstPlayerName} vs ${secondPlayerName} - ${formatVietnamLiveTitleTime()}`;
      ...
      const resolveIngestion = (session: any) => {
        ...
        const normalizeYouTubeIngestUrlForFfmpeg = (value: string) => {
          const cleanValue = String(value || DEFAULT_YOUTUBE_RTMP_URL).trim().replace(/\/+$/g, '');
          // v30: keep the exact YouTube ingest URL returned by the backend.
          // v29 forced RTMPS -> RTMP, but the stream still stayed at
          // streamStatus=ready. Use YouTube's provided RTMPS endpoint again
          // while the native resolver is fixed to prefer the real WinGet
          // ffmpeg.exe that successfully captured camtest.mp4 on this machine.
          return cleanValue;
        };

        if (streamUrl && streamName) {
          return {rtmpUrl: normalizeYouTubeIngestUrlForFfmpeg(streamUrl), streamKey: streamName};
        }
      ...
      const prepareWindowsFfmpegYouTubeLive = async () => {
        ...
        const waitForCameraPreviewReady = async () => {
          const startedAt = Date.now();
          while (Date.now() - startedAt < 3500) {
            if (isCameraReadyRef.current) {
              console.log('[LiveCameraWarmup]', {ready: true, waitedMs: Date.now() - startedAt});
              return true;
            }
            await new Promise(resolve => setTimeout(resolve, 150));
          }
          console.log('[LiveCameraWarmup]', {ready: false, waitedMs: Date.now() - startedAt});
          return false;
        };

        const releaseWindowsPreviewBeforeFfmpeg = async (stage: string) => {
          // v51: Windows live now keeps app preview and live upload running together.
          // Native live capture uses its own MediaCapture frame reader in SharedReadOnly
          // mode, so we should not unmount or release the preview camera anymore.
          setYoutubeLivePreparing(true);
          setYoutubeLivePreviewActive(true);
          setYouTubeNativeCameraLock(true);
          setYouTubeSourceLock(lockedLiveSource);

          console.log('[LiveCameraSharedPreview] keep preview mounted while starting ffmpeg', {
            stage,
            source: lockedLiveSource,
          });
        ...
        try {
          await waitForCameraPreviewReady();
          await releaseWindowsPreviewBeforeFfmpeg('before-youtube-create');
          ...
          liveResponse = await createYouTubeLiveSession({
            ...
            scheduledStartTime: new Date(Date.now() + 1500).toISOString(),
          ...
          const windowsLiveConfig: WindowsFfmpegLiveConfig = {
            ...
            // v71 stable live first: disable microphone capture during startup.
            // Keep a stable AAC track with FFmpeg lavfi anullsrc.
            audioDeviceName: '',
            useAudio: true,
          ...
          console.log('[LiveCameraRelease] before ffmpeg camera-only start', {
            reason: 'shared-preview-mode-v51',
          });
          ...
          setYoutubeLivePreparing(false);
          // v34: keep the app camera preview unmounted while FFmpeg owns the webcam.
          // This prevents MediaCapture from stealing the USB camera back and killing
          // the YouTube ingest. The app should show live-state UI instead of trying
          // to preview the same camera twice.
          setYoutubeLivePreviewActive(true);
          setYouTubeNativeCameraLock(true);
          // Match was already started before this async live setup began.
          // Keep gameplay independent from live setup completion.
          setActiveGameplaySessionSync({
            matchSessionId: matchSessionIdRef.current,
            webcamFolderName,
            savedAt: Date.now(),
            source: 'windows-ffmpeg-oauth-live-started-after-match',
          });

          // v28: YouTube often stays at “Sắp diễn ra” until the backend polls
          // liveStreams.status.streamStatus and explicitly transitions the bound
          // broadcast to live. The backend /status endpoint already contains the
          // transition logic, but the Windows app was not calling it after FFmpeg
          // started. Poll it in the background so the match can keep running while
          // YouTube moves ready -> live once ingest becomes active.
          const pollBroadcastId = String(activeYouTubeBroadcastIdRef.current || '').trim();
          if (pollBroadcastId) {
            (async () => {
              for (let attempt = 1; attempt <= 30; attempt += 1) {
                if (activeYouTubeBroadcastIdRef.current !== pollBroadcastId) {
                  return;
                }

                try {
                  const status: any = await getYouTubeLiveStatus(pollBroadcastId);
                  ...
                  console.log('[YouTube Live Status Poll]', {
                    attempt,
                    broadcastId: pollBroadcastId,
                    broadcastStatus,
                    streamStatus,
                  ...
                  if (Boolean((status as any)?.redundantTransition)) {
                    console.log('[YouTube Live Status Poll] redundant transition marker; keep waiting for real YouTube status', {
                      attempt,
                      broadcastId: pollBroadcastId,
                    });
                  }
                  ...
                  if (
                    ffmpegStatus?.status === 'stopped' ||
                    ffmpegStatus?.status === 'error'
                  ) {
                    const ffmpegMessage =
                      ffmpegStatus?.stderrSummary ||
                      ffmpegStatus?.error ||
                      'FFmpeg đã dừng trước khi YouTube nhận được tín hiệu live.';
                    console.log('[YouTube Live Status Poll] FFmpeg stopped before YouTube live', {
                      attempt,
                      broadcastId: pollBroadcastId,
                      broadcastStatus,
                      streamStatus,
                    ...
                    try {
                      await stopWindowsFfmpegYouTubeLive('ffmpeg-stopped-before-youtube-live');
                    } catch {}
                    try {
                      await stopYouTubeLiveSession(pollBroadcastId);
                    } catch (stopError) {
                      console.log('[YouTube Live Status Poll] stop failed after FFmpeg died', stopError);
                    }

                    showYouTubeLiveFailure(
                      null,
                      `Không live được YouTube vì FFmpeg không mở được camera/RTMP. Chi tiết: ${ffmpegMessage}`,
                    );
                    return;
                  }
                } catch (statusError: any) {
                  const message = String(statusError?.message || statusError?.payload?.message || statusError || '');
                  if (message.toLowerCase().includes('redundant transition')) {
                    console.log('[YouTube Live Status Poll] redundant transition; keep waiting for real live status', {
                      attempt,
                      broadcastId: pollBroadcastId,
                    });
                    await new Promise(resolve => setTimeout(resolve, attempt <= 10 ? 800 : 1200));
                    continue;
                  }
                  console.log('[YouTube Live Status Poll] failed', {
                    attempt,
                    message,
                  });
                }
          ...
          setYoutubeLivePreparing(false);
          setYoutubeLivePreviewActive(false);
          // Do not stop the match if livestream setup fails. The match and timer are independent.
          setYouTubeSourceLock(null);
  ...
  }, [isPaused, _resetCountdown, startNewGameAfterViolate, youtubeLiveNativeMode, isWindowsYouTubeLiveOnly]);
  ...
  const scheduleWindowsYouTubeLiveDrainStop = useCallback(
    (broadcastId: string, reason = 'end-match') => {
      const cleanBroadcastId = String(broadcastId || '').trim();
      const drainStopId = ++pendingWindowsYouTubeDrainStopId;

      console.log('[YouTube Live DrainStop] scheduled', {
        reason,
        broadcastId: cleanBroadcastId,
        drainMs: WINDOWS_YOUTUBE_LIVE_END_DRAIN_MS,
      });

      setTimeout(() => {
        void (async () => {
          if (drainStopId !== pendingWindowsYouTubeDrainStopId) {
            console.log('[YouTube Live DrainStop] skipped because a newer live session started', {
              reason,
              broadcastId: cleanBroadcastId,
              drainStopId,
              currentDrainStopId: pendingWindowsYouTubeDrainStopId,
            });
            return;
          }

          try {
            await stopWindowsFfmpegYouTubeLive(`${reason}-drain-tail`);
          } catch (ffmpegStopError) {
            console.log('[YouTube Live DrainStop] FFmpeg stop failed:', ffmpegStopError);
          }

          if (cleanBroadcastId) {
            try {
              await stopYouTubeLiveSession(cleanBroadcastId);
              console.log('[YouTube Live DrainStop] stopped broadcast after tail:', cleanBroadcastId);
            } catch (youtubeStopError) {
              console.log('[YouTube Live DrainStop] stop broadcast failed:', youtubeStopError);
            }
          }
        })();
      }, WINDOWS_YOUTUBE_LIVE_END_DRAIN_MS);
    },
    [],
  );
    ...
    },
    {
      ...
      onPress: async () => {
        ...
        try {
          ...
          if (Platform.OS === 'windows' && shouldUseYouTubeLive) {
            // YouTube player can be 10-20s behind even when ingest is current.
            // Keep FFmpeg pushing the last live camera frames for a short tail,
            // then stop the broadcast. This prevents viewers from losing the last
            // delayed seconds when the operator exits the match screen.
            scheduleWindowsYouTubeLiveDrainStop(activeYouTubeBroadcastId, 'end-match');
          } else {
            await stopYouTubeNativeLive();
  ...
  isWindowsYouTubeLiveOnly,
  shouldUseYouTubeLive,
  scheduleWindowsYouTubeLiveDrainStop,
]);
